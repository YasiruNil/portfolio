      ...
      <main id="main-content" tabIndex="-1" className="flex-1">
        ...
        <section className="relative min-h-screen pt-32 flex items-center overflow-hidden" aria-labelledby="hero-heading">
          ...
          <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop grid grid-cols-1 md:grid-cols-12 gap-gutter items-center relative z-10">
            ...
            >
              <motion.span
                className="inline-block border border-color-accent-cyan/30 bg-color-accent-cyan/5 text-color-accent-cyan font-label-mono text-xs py-2 px-5 rounded-full self-start tracking-widest"
                id="status-badge"
              ...
              <motion.h1
                id="hero-heading"
                className="font-display-lg text-5xl md:text-7xl text-color-on-surface leading-[1.05] tracking-tighter"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.3, duration: 0.6 }}
              >
                Yasiru Nilanga Damboragama <br />
                <span className="text-color-accent-cyan opacity-90">Software Engineer</span>
              </motion.h1>
              <motion.p
                className="font-body-lg text-lg text-color-on-surface-variant max-w-xl leading-relaxed"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
        ...
        <section className="py-24 bg-color-surface-container-lowest border-y border-white/5" aria-labelledby="stack-heading">
          <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop text-center">
            ...
            <div className="flex flex-wrap justify-center items-center gap-16 md:gap-24" role="list">
              {['hub', 'javascript', 'cloud', 'grid_view'].map((icon, i) => (
                <motion.div
                  key={i}
